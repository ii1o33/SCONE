  ...
  subroutine splitConcave(self, edges, faces, vertices, newEdges, convexElements, newFaces, newVertices)
    ...
    do i = 1, size(notches)
      ...
      do j = 1, size(faceToSplitIdxs)
        ...
        do k = 1, size(faceVertexIdxs)
          if (any(notchVertexIdxs == faceVertexIdxs(k))) then
            dotProducts(k) = ZERO
            cycle

          end if
          dotProducts(k) = dot_product(normalDifference, notchVertexCoord - vertices % getVertexCoordinates(faceVertexIdxs(k)))
        ...
        ! Create the new faces (for vertices with negative dot product) in the newFaces shelf.
        nFaces = nFaces + 1
        type = 'Polygon'
        ...
        ! Create the new faces (for vertices with positive dot product) in the newFaces shelf.
        nFaces = nFaces + 1
        type = 'Polygon'
      ...
      ! {generalisation complete from 4}
      ! Create a new face corresponding to the cut plane.
      nFaces = nFaces + 1
      allocate(newFaceVertexIdxs(0))
      call append(newFaceVertexIdxs, notchVertexIdxs(1))

      allocate(array(nVertices - nInitialVertices))
      do j = 1, size(array)
        ! this is the entire set of vertices forming the face
        array(j) = nInitialVertices + j

      end do

      outerLoop: do j = 1, nVertices - nInitialVertices + 1   !{loop through vertices (notch vertex > all intersections) 1}
        vertexEdgeIdxs = newVertices % getVertexEdgeIdxs(newFaceVertexIdxs(j))
        ! {looping through all the associated edges. Aim: find edges whose two vertices are the candidate vertices}
        ! {1 and 2 together allows finding the sequence of candidate vertices by following a series of relevant edges}
        ! {there is at most one edge that has not been explored and is relevant - so it is general}
        do k = 1, size(vertexEdgeIdxs) 
          currEdgeVertexIdxs = newEdges % getEdgeVertexIdxs(vertexEdgeIdxs(k))
          diff = findDifferent(currEdgeVertexIdxs, newFaceVertexIdxs(j))
          if (any(array == diff)) then   !{if current edge contains candidate vertex}
            if (any(newFaceVertexIdxs == diff)) cycle   !{if this edge has already been considered} 
      ...
      call append(newFaceVertexIdxs, notchVertexIdxs(2)) !{close the sequence by adding the other notch vertex}
      ...
      call append(positiveIdxs, nFaces)   ! {cut face belongs to the both elements}
